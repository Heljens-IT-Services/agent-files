...
4. Mit `code_diff-review` den vollständigen Release-Diff prüfen und nur bei `versandbereit` oder `versandbereit mit Hinweisen` fortfahren.
...
6. Mit `github_pr-erstellen` einen vorhandenen Pull Request von `develop` nach `main` wiederverwenden oder mit `main` als expliziter Basis und `develop` als explizitem Head erstellen. `/release` gilt als ausdrückliche Anweisung, den Pull Request als Ready for Review bereitzustellen.
...
- Bei `code_diff-review` mit Bewertung `nicht versandbereit` zur betroffenen Umsetzung auf einem separaten Arbeitsbranch zurückkehren; `develop` im Release-Workflow nicht direkt verändern.
